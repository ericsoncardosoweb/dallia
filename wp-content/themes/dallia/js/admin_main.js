...
/** Product Category - Breadcrumbs Background Image **/
jQuery(document).ready(function($){
	"use strict";
	
	/* Only show the "remove image" button when needed */
	if ( ! $( '#product_cat_bg_breadcrumbs_id' ).val() ) {
		$( '.product_cat_bg_breadcrumbs_remove_button' ).hide();
	}

	/* Uploading files */
	var file_frame;

	$( document ).on( 'click', '.product_cat_bg_breadcrumbs_upload_button', function( event ) {
		...
		file_frame.on( 'select', function() {
			...
			$( '#product_cat_bg_breadcrumbs_id' ).val( attachment.id );
			$( '#product_cat_bg_breadcrumbs' ).find( 'img' ).attr( 'src', attachment.sizes.thumbnail.url );
			$( '.product_cat_bg_breadcrumbs_remove_button' ).show();
		});
	...
	$( document ).on( 'click', '.product_cat_bg_breadcrumbs_remove_button', function() {
		$( '#product_cat_bg_breadcrumbs' ).find( 'img' ).remove();
		$( '#product_cat_bg_breadcrumbs' ).append( '<img src="' + $('#wc_placeholder_img_src').val() + '" class="woocommerce-placeholder wp-post-image" width="60" height="60" alt="Placeholder" />' );
		$( '#product_cat_bg_breadcrumbs_id' ).val( '' );
		$( '.product_cat_bg_breadcrumbs_remove_button' ).hide();
		return false;
	});
});