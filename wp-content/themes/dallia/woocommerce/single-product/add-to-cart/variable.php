...
$attr_dropdown = !isset($smof_data['ts_prod_attr_dropdown']) || ( isset($smof_data['ts_prod_attr_dropdown']) && $smof_data['ts_prod_attr_dropdown'] );
$select_class = '';
if( !$attr_dropdown ){
	$select_class = 'hidden';
}
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->id ); ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php esc_html_e( 'This product is currently out of stock and unavailable.', 'gon' ); ?></p>
	<?php else : ?>
				...
				<?php foreach ( $attributes as $attribute_name => $options ) : ?>
					<tr>
						...
						<td class="value">
								...
								<div class="ts-product-attribute">
									...
									if ( taxonomy_exists( $attribute_name ) ) {
										...
										foreach ( $terms as $term ) {
											...
											if( $is_attr_color ){
												if( absint($datas['ts_color_image']) > 0 ){
													echo '<a href="#">' . wp_get_attachment_image( absint($datas['ts_color_image']), 'ts_prod_color_thumb', true, array('title'=>$term->name, 'alt'=>$term->name) ) . '</a>';
												}
												else{
										...
										foreach ( $options as $option ) {
											...
											$class .= ' option';
											echo '<div data-value="' . esc_attr( sanitize_title( $option ) ) . '" class="' . $class . '"><a href="#">' . esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) ) . '</a></div>';
										}
							...
							echo end( $attribute_keys ) === $attribute_name ? '<a class="reset_variations" href="#">' . esc_html__( 'Clear selection', 'gon' ) . '</a>' : '';
						?>
						</td>
					</tr>
		        <?php endforeach;?>
