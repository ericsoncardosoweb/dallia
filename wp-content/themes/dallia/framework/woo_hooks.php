...
add_action('woocommerce_before_shop_loop_item_title', 'ts_template_loop_product_thumbnail', 10);
add_action('woocommerce_after_shop_loop_item_title', 'ts_template_loop_product_label', 1);

add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_categories', 10);
add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_product_title', 20);
add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_product_sku', 30);
add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_short_description', 40); 
add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_short_description_listview', 65); 
...
add_action('woocommerce_after_shop_loop_item', 'ts_template_loop_add_to_cart', 70); 

add_action('woocommerce_before_shop_loop', 'ts_shop_top_content_widget_area_button', 25);

add_filter('loop_shop_per_page', 'ts_change_products_per_page_shop' ); 
add_filter('woocommerce_product_get_rating_html', 'ts_get_empty_rating_html', 10, 2);
...
function ts_template_loop_product_thumbnail(){
	...
	if( defined( 'YITH_INFS' ) && is_shop() ){ /* Compatible with YITH Infinite Scrolling */
		$lazy_load = false;
	}
	
	$prod_galleries = $product->get_gallery_attachment_ids();
	
	$has_back_image = (isset($smof_data['ts_effect_product']) && (int)$smof_data['ts_effect_product'] == 0)?false:true;
	
	if( !is_array($prod_galleries) || ( is_array($prod_galleries) && count($prod_galleries) == 0 ) ){
		$has_back_image = false;
	}
	 
	if( wp_is_mobile() || TS_IS_MOBILE || TS_IS_TABLET ){
		$has_back_image = false;
	}
	
	$image_size = apply_filters('ts_loop_product_thumbnail', 'shop_catalog');
...
function ts_template_loop_add_to_cart(){
	...
	if( $smof_data['ts_enable_catalog_mode'] ){
		return;
	}
...
function ts_template_loop_short_description(){
	global $product, $post, $smof_data;
	$has_grid_list = class_exists('TS_Grid_List');
	$grid_limit_words = absint($smof_data['ts_prod_cat_grid_desc_words']);
	$show_grid_desc = $smof_data['ts_prod_cat_grid_desc'];
	...
	<div class="short-description">
		<?php ts_the_excerpt_max_words($grid_limit_words, '', true, '', true); ?>
	</div>
	<?php
	else:
		if( $show_grid_desc ):
...
function ts_template_loop_short_description_listview(){
	...
	?>
		<div class="short-description list" style="display: none" >
			<?php ts_the_excerpt_max_words($list_limit_words, '', true, '', true); ?>
		</div>
	<?php
	endif;
}
...
function ts_change_products_per_page_shop(){
	global $smof_data;
    if( is_tax('product_cat') || is_tax('product_tag') || is_post_type_archive('product') ){
        if( isset($smof_data["ts_prod_cat_per_page"]) && absint($smof_data["ts_prod_cat_per_page"]) > 0){
            return absint($smof_data["ts_prod_cat_per_page"]);
        }
    }
}

function ts_get_empty_rating_html( $rating_html, $rating ){
	if( $rating == 0 ){
...
function ts_shop_top_content_widget_area_button(){
	global $smof_data;
	if( is_active_sidebar('product-category-top-content') && $smof_data['ts_prod_cat_top_content'] ){
	?>
...
/* Add hook */
add_action('ts_before_product_image', 'ts_template_loop_product_label', 10);

add_action('woocommerce_single_product_summary', 'ts_template_single_navigation', 1);
add_action('woocommerce_single_product_summary', 'woocommerce_template_single_price', 2);
add_action('woocommerce_single_product_summary', 'ts_template_single_availability', 3);
add_action('woocommerce_single_product_summary', 'woocommerce_template_single_rating', 5);
add_action('woocommerce_single_product_summary', 'ts_template_single_sku', 6);
add_action('woocommerce_single_product_summary', 'ts_template_single_meta', 60);
add_action('woocommerce_single_product_summary', 'woocommerce_template_single_sharing', 70);

add_action('woocommerce_after_single_product_summary', 'ts_product_ads_banner', 12);

add_action('woocommerce_share', 'ts_template_before_single_social_sharing', 9);
add_action('woocommerce_share', 'ts_template_social_sharing', 10);
add_action('woocommerce_share', 'ts_template_after_single_social_sharing', 11);
...
add_filter('woocommerce_available_variation', 'ts_variable_product_price_filter', 10, 3);
...
if( !is_admin() ){ /* Fix for WooCommerce Tab Manager plugin */
	add_filter( 'woocommerce_product_tabs', 'ts_product_remove_tabs', 999 );
	add_filter( 'woocommerce_product_tabs', 'ts_add_product_custom_tab', 90 );
}
/*** End Product ***/

function ts_template_single_navigation(){
...
function ts_template_before_single_social_sharing(){
	?>
	<div class="social-sharing">
		...
		<div class="email">
			<a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>" target="_blank">
...
function ts_template_after_single_social_sharing(){
	?>
	</div>
	<?php
}

function ts_template_single_meta(){
	global $product, $post, $smof_data;
	$cat_count = sizeof( get_the_terms( $post->ID, 'product_cat' ) );
	$tag_count = sizeof( get_the_terms( $post->ID, 'product_tag' ) );
	
	echo '<div class="meta-wrapper">';
		if( $smof_data['ts_prod_cat'] ){
			echo $product->get_categories( ', ', '<div class="cats-link"><span>' . esc_html__( 'Categories:', 'gon' ) . '</span><span class="cat-links">', '</span></div>' );
		}
		if( $smof_data['ts_prod_tag'] ){
			echo $product->get_tags( ', ', '<div class="tags-link"><span>' . esc_html__( 'Tags:', 'gon' ) . '</span><span class="tag-links">', '</span></div>' );	
		}
	echo '</div>';
}
...
function ts_template_single_availability(){
	...
	$product_stock = $product->get_availability();
	$availability_text = empty($product_stock['availability'])?esc_html__('In Stock', 'gon'):esc_attr($product_stock['availability']);
	?>	
...
/*** Product tab ***/
function ts_product_remove_tabs( $tabs = array() ){
	global $smof_data;
	if( !$smof_data['ts_prod_tabs'] ){
		return array();
	}
	return $tabs;
}

function ts_add_product_custom_tab( $tabs = array() ){
	global $smof_data, $post;
	
	$custom_tab_title = $smof_data['ts_prod_custom_tab_title'];
	...
	if( $smof_data['ts_prod_custom_tab'] ){
		$tabs['ts_custom'] = array(
			'title'    	=> esc_html( $custom_tab_title )
			,'priority' => 90
			,'callback' => "ts_product_custom_tab_content"
		);
	} 
	return $tabs;
}

function ts_product_custom_tab_content(){
	global $smof_data, $post;
	
	$custom_tab_content = $smof_data['ts_prod_custom_tab_content'];
...
/* Ads Banner */
function ts_product_ads_banner(){
	global $smof_data;
	
	if( $smof_data['ts_prod_ads_banner'] ){
		$ads_banner_content = $smof_data['ts_prod_ads_banner_content'];
...
function ts_variable_product_price_filter( $value, $object = null, $variation = null ){
	if( $value['price_html'] == '' ){
...
* End tag:   	10005
**************************************************************/
add_action('woocommerce_after_shop_loop_item_title', 'ts_template_loop_add_to_cart', 10001 );
function ts_product_group_button_start(){
	global $smof_data;
	$num_icon = 0;
	
	if( has_action('woocommerce_after_shop_loop_item_title', 'ts_template_loop_add_to_cart') && !$smof_data['ts_enable_catalog_mode'] ){
		$num_icon++;
	}
	if( class_exists('TS_Quickshop') ){
		$num_icon++;
	}
...
function ts_product_group_button_end(){
	echo "</div>";
}

add_action('woocommerce_after_shop_loop_item_title', 'ts_product_group_button_start', 10000 );
add_action('woocommerce_after_shop_loop_item_title', 'ts_product_group_button_end', 10005 );
...
if( class_exists('YITH_WCWL') ){
	function ts_add_wishlist_button_to_product_list(){
		...
		echo '<div class="button-in wishlist add-to-wishlist-'.$product->id.' '.$added_class.'">';
			echo '<a href="' . esc_url( add_query_arg( 'add_to_wishlist', $product->id ) )
				. '" data-product-id="' . $product->id . '" data-product-type="' . $product->product_type 
				. '" class="add_to_wishlist wishlist" ><i class="fa fa-heart"></i><span class="ts-tooltip button-tooltip">'.esc_html__('Wishlist', 'gon').'</span></a>';
	...
	add_action( 'woocommerce_after_shop_loop_item_title', 'ts_add_wishlist_button_to_product_list', 10002 );
	add_action( 'woocommerce_after_shop_loop_item', 'ts_add_wishlist_button_to_product_list', 80 );
}
...
if( class_exists('YITH_Woocompare') && get_option('yith_woocompare_compare_button_in_products_list') == 'yes' ){
	...
	if( $yith_woocompare->is_frontend() || $is_ajax ) {
		...
		function ts_add_compare_button_to_product_list(){
			if( wp_is_mobile() )
				return;
			global $yith_woocompare, $product;
			echo '<div class="button-in compare">';
			echo '<a class="compare" href="'.$yith_woocompare->obj->add_product_url( $product->id ).'" data-product_id="'.$product->id.'">'.get_option('yith_woocompare_button_text').'</a>';
			echo '</div>';
		}
		add_action( 'woocommerce_after_shop_loop_item_title', 'ts_add_compare_button_to_product_list', 10003 );
		add_action( 'woocommerce_after_shop_loop_item', 'ts_add_compare_button_to_product_list', 70 );
		
		add_filter( 'option_yith_woocompare_button_text', 'ts_compare_button_text_filter', 99 );
		function ts_compare_button_text_filter( $button_text ){
...
if( isset($_GET['action']) && $_GET['action'] == 'yith-woocompare-view-table' ){
	add_action('wp_head', 'ts_add_custom_style_compare_popup');
}
function ts_add_custom_style_compare_popup(){
	global $smof_data;
	echo '<link rel="stylesheet" type="text/css" media="all" href="'.get_template_directory_uri().'/css/reset.css" />';
	echo '<link rel="stylesheet" type="text/css" media="all" href="'.get_template_directory_uri().'/style.css" />';
	echo '<link rel="stylesheet" type="text/css" media="all" href="'.get_template_directory_uri().'/css/font-awesome.css" />';
	
	if( $smof_data['ts_enable_rtl'] ){
		echo '<link rel="stylesheet" type="text/css" media="all" href="'.get_template_directory_uri().'/css/rtl.css" />';
	}
	
	/* Add custom css for iframe */
	ts_add_header_dynamic_css( true );
	
	/* Register google font for iframe */
	ts_register_google_font( true );
}
/*** End General hook ***/
...
add_action('woocommerce_proceed_to_checkout', 'ts_cart_continue_shopping_button', 20);

/* Continue Shopping button */
function ts_cart_continue_shopping_button(){
