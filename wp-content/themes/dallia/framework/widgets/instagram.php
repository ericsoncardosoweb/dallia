...
if(!class_exists('TS_Instagram_Widget')){
	class TS_Instagram_Widget extends WP_Widget {
		...
		function TS_Instagram_Widget(){
			$widgetOps = array('classname' => 'ts-instagram-widget', 'description' => esc_html__('Display your photos from Instagram', 'gon'));
			parent::__construct('ts_instagram', esc_html__('TS - Instagram', 'gon'), $widgetOps);
		}
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('username'); ?>"><?php esc_html_e('Username', 'gon'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of photos', 'gon'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('column'); ?>"><?php esc_html_e('Column', 'gon'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('size'); ?>"><?php esc_html_e('Size', 'gon'); ?> </label>
				<select class="widefat" id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>" >
					...
					<option value="original" <?php selected($instance['size'], 'original'); ?> >Original</option>
				</select>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('target'); ?>"><?php esc_html_e('Target', 'gon'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('cache_time'); ?>"><?php esc_html_e('Cache time (hours)', 'gon'); ?> </label>
		...
		function scrape_instagram( $username, $slice = 9 ) {
			...
			if ( is_wp_error( $remote ) ){
				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'gon' ) );
			}

			if ( 200 != wp_remote_retrieve_response_code( $remote ) ){
				return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'gon' ) );
			}
			...
			if ( ! $insta_array ){
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'gon' ) );
			}

			if ( isset( $insta_array['entry_data']['ProfilePage'][0]['user']['media']['nodes'] ) ) {
				$images = $insta_array['entry_data']['ProfilePage'][0]['user']['media']['nodes'];
			} else {
				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'gon' ) );
			}

			if ( ! is_array( $images ) ){
				return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'gon' ) );
			}
			...
			foreach ( $images as $image ) {
				...
				$caption = __( 'Instagram Image', 'gon' );
			...
			if ( ! empty( $instagram ) ) {
				...
				return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'gon' ) );
			}
		}
	}
}
