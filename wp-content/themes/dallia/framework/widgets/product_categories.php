...
if(!class_exists('TS_Product_Categories_Widget')){
	class TS_Product_Categories_Widget extends WP_Widget {
		...
		function TS_Product_Categories_Widget() {
			$widgetOps = array('classname' => 'ts-product-categories-widget', 'description' => esc_html__('Display Your Product Categories', 'gon'));
			parent::__construct('ts_product_categories', esc_html__('TS - Product Categories', 'gon'), $widgetOps);
		}
		...
		function widget( $args, $instance ) {
			...
			$number 			= absint($instance['number']) == 0 ? '' : absint($instance['number']);		
		...
		function form( $instance ) { 
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_post_count'); ?>"><?php esc_html_e('Show post count', 'gon'); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_sub_cat'); ?>"><?php esc_html_e('Show sub categories', 'gon'); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('hide_empty'); ?>"><?php esc_html_e('Hide empty categories', 'gon'); ?></label>
			</p>
			<p>
				<label><?php esc_html_e('Select categories', 'gon'); ?></label>
				...
				<span class="description"><?php esc_html_e("Don't select to show all", "gon"); ?></span>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of parent categories to show', 'gon'); ?></label>
				<input class="widefat" type="number" min="0" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" value="<?php echo esc_attr($instance['number']); ?>" />
				<span class="description"><?php esc_html_e('Input 0 to show all', 'gon'); ?></span>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php esc_html_e('Order by', 'gon'); ?></label>
				<select class="widefat" id="<?php echo $this->get_field_id('orderby'); ?>" name="<?php echo $this->get_field_name('orderby'); ?>" >
					<option value="name" <?php selected($instance['orderby'], 'name'); ?> ><?php esc_html_e('Name', 'gon'); ?></option>
					<option value="slug" <?php selected($instance['orderby'], 'slug'); ?> ><?php esc_html_e('Slug', 'gon'); ?></option>
					<option value="count" <?php selected($instance['orderby'], 'count'); ?> ><?php esc_html_e('Number product', 'gon'); ?></option>
					<option value="rand" <?php selected($instance['orderby'], 'rand'); ?> ><?php esc_html_e('Random', 'gon'); ?></option>
					<option value="none" <?php selected($instance['orderby'], 'none'); ?> ><?php esc_html_e('None', 'gon'); ?></option>
				</select>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('order'); ?>"><?php esc_html_e('Order', 'gon'); ?></label>
				<select class="widefat" id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>" >
					<option value="asc" <?php selected($instance['orderby'], 'asc'); ?> ><?php esc_html_e('Ascending', 'gon'); ?></option>
					<option value="desc" <?php selected($instance['orderby'], 'desc'); ?> ><?php esc_html_e('Descending', 'gon'); ?></option>
				</select>
			</p>
