...
global $ts_mega_menu;
$ts_mega_menu = new TS_Mega_Menu();

/* Custom Html Menu Item */
if( !class_exists('TS_Custom_Mega_Menu') ){
	class TS_Custom_Mega_Menu extends Walker_Nav_Menu{
		function __construct(){
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
			global $smof_data, $ts_mega_menu;
			$num_widget = isset($smof_data['ts_menu_num_widget'])?(int)$smof_data['ts_menu_num_widget']:0;
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)','gon' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__('%s (Pending)','gon'), $item->title );
			}
					...
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','gon'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										esc_url(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','gon'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','gon'); ?>" href="<?php
							...
							?>"><?php esc_html_e( 'Edit Menu Item','gon' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Navigation Label','gon' ); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Title Attribute','gon' ); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Open link in a new window/tab','gon' ); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'CSS Classes (optional)','gon' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Link Relationship (XFN)','gon' ); ?><br />
							<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
						</label>
					</p>
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Description','gon' ); ?><br />
							<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
							<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','gon'); ?></span>
						</label>
					</p>        
					<?php
					/*
					...
					<p class="field-ts-is-megamenu description description-wide ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-is-megamenu-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e('Enable Mega Menu', 'gon'); ?>
						</label>
					</p>		
					...
					<?php if( $num_widget > 0):?>
							<p class="field-wide-widget description description-thin ts-custom-menu ts-active-lv0 ts-active-lv1">
								<label for="edit-menu-item-ts-sidebars-<?php echo esc_attr($item_id); ?>">
									<span class="description"><?php esc_html_e('Display A Widget Area','gon'); ?></span>
									<?php echo $ts_mega_menu->select_sidebar_html($item_id, $num_widget); ?>
								</label>
							</p>
					<?php endif ?>	
					...
					<p class="field-ts-static-html description description-wide ts-custom-menu ts-active-lv0 ts-active-lv1">
						<label for="edit-menu-item-ts-static-html-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Static HTML','gon' ); ?><br />
					...
					<p class="field-ts-thumbnail description description-thin ts-custom-menu ts-active-lv0">
						<label>
							<?php 
								$ts_thumbnail = wp_get_attachment_url( $item->ts_thumbnail_id );
								esc_html_e( 'Set Thumbnail', 'gon' ); 
							?>
							<br />
					...
					<p class="field-ts-background-repeat description description-thin ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-background-repeat-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Background Repeat', 'gon' ); ?><br />
					...
					<p class="field-ts-background-position description description-thin ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-background-position-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Background Position', 'gon' ); ?><br />
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( esc_html__('Original: %s','gon'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php esc_html_e('Remove','gon'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','gon'); ?></a>
					</div>
...
if( !class_exists('TS_Walker_Nav_Menu') ){
	class TS_Walker_Nav_Menu extends Walker_Nav_Menu{
		...
		function __construct(){
			global $smof_data;
			if( isset($smof_data['ts_menu_num_widget']) ){
				$this->menu_config['num_widget'] = (int)$smof_data['ts_menu_num_widget'];
			}
			if( isset($smof_data['ts_menu_thumb_width']) ){
				$this->menu_config['thumb_width'] = (int)$smof_data['ts_menu_thumb_width']; 
			}
			if( isset($smof_data['ts_menu_thumb_height']) ){
				$this->menu_config['thumb_height'] = (int)$smof_data['ts_menu_thumb_height'];
			}
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ){
			...
			if( $depth === 0 || ($depth > 0 && !$this->parent_megamenu) ){
				...
				$icon_html = '';
				if( $thumbnail_id > 0 && $this->parent_megamenu ){
					$icon_html = '<span class="menu-icon">'.wp_get_attachment_image( $thumbnail_id, 'ts_menu_icon_thumb', false).'</span>';
				}
