...
class TS_Product_Filter_By_Color{
	...
	function edit_color_attribute( $term, $taxonomy ){
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Color', 'gon' ); ?></label></th>
			<td>
				...
				<span class="description"><?php esc_html_e('Use color picker to pick one color.','gon'); ?></span>
			</td>
		</tr>

		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail Image', 'gon' ); ?></label></th>
			<td>
			...
			jQuery(document).ready(function(){
				...
				jQuery(document).on( 'click', '.ts_color_upload_image_button', function( event ){
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php esc_html_e( 'Choose an image', 'gon' ); ?>',
						button: {
							text: '<?php esc_html_e( 'Use image', 'gon' ); ?>',
						},
						multiple: false
					});
	...
	function add_color_attribute(){
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Color', 'gon' ); ?></label>
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Thumbnail Image', 'gon' ); ?></label>
	...
	function save_color_fields( $term_id, $tt_id, $taxonomy ){
		if( isset($_POST['ts_color_color'], $_POST['ts_color_image']) ){
			...
			update_metadata( 'woocommerce_term', $term_id, 'ts_product_color_config', serialize($datas) );
		}
	}
	
	function remove_color_fields( $term_id, $tt_id, $taxonomy ){
		delete_metadata( 'woocommerce_term', $term_id, 'ts_product_color_config' );
	}
}
