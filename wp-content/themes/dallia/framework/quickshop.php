		...
		class TS_Quickshop{
			...
			function add_hook(){
				...
				if( !$smof_data['ts_enable_catalog_mode'] ){
					add_action('ts_quickshop_single_product_summary', 'woocommerce_template_single_add_to_cart', 60); 
				}
			...
			function load_quickshop_content_callback(){
				...
				add_filter( 'ts_woocommerce_review_link_filter', array($this, 'filter_review_link'), 10 );
				
				$_wrapper_class = "ts-quickshop-wrapper product type-{$product->product_type}";
				ob_start();	
				?>		
				<div itemscope itemtype="http://schema.org/Product" id="product-<?php echo get_the_ID();?>" <?php post_class( apply_filters('single_product_wrapper_class',$_wrapper_class  ) ); ?>>
					...
					<div class="summary entry-summary">
						<?php do_action('ts_quickshop_single_product_title'); ?>
						<?php do_action('ts_quickshop_single_product_summary'); ?>
					</div>
				...
				remove_filter( 'ts_woocommerce_review_link_filter', array($this, 'filter_review_link'), 10 );
